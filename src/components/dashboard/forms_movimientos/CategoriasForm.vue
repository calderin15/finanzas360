<template>
    <article id="form_categorias" class="mt-6 mb-6 duration-500">
        <p class="mt-10 mb-4 text-xl font-medium dark:text-white">Completa los siguientes campos para crear su categoria</p>
        <div class="relative z-0 w-full mb-6 group">
            <input v-model="categorias.tipo" type="text" name="tipo" id="tipo"
            ...
            <label for="tipo"
                class="peer-focus:font-medium absolute text-sm text-gray-500 dark:text-gray-400 duration-300 transform -translate-y-6 scale-75 top-3 -z-10 origin-[0] peer-focus:left-0 peer-focus:text-green-600 peer-focus:dark:text-green-500 peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-75 peer-focus:-translate-y-6">Tipo</label>
        </div>
        <div class="relative z-0 w-full mb-6 group">
            <input v-model="categorias.nombre" type="text" name="nombre" id="nombre"
                class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-green-500 focus:outline-none focus:ring-0 focus:border-green-600 peer"
                placeholder=" " required />
            <label for="nombre"
        ...
        <div class="relative z-0 w-full mb-6 group">
            ...
            <textarea v-model="categorias.descripcion" id="message" rows="4"
        ...
        <button type="button" @click="newCategoria"
...
export default {
    ...
    methods: {
        newCategoria() {
            ...
            this.categorias.id_user = usuario.id_user;
            axios({
                method: 'post',
                url: 'https://apigenerator.dronahq.com/api/I4p61D3-/categorias',
                data: this.categorias,
                responseType: 'json',
            })
                .then(response => {
                    this.categorias.nombre = null
                    this.categorias.descripcion = null
                    this.categorias.tipo = null
